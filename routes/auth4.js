...
const { issueVerificationCode, checkVerificationCode } = require('../utils/emailVerification');
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!user.email_verified) {
      // Only self-signup patients ever have this false (staff-created and
      // staff-invited accounts default to verified). Password was correct,
      // so this is genuinely them — send them to verify rather than a
  ...
  try {
    ...
    const userResult = await client.query(
      `INSERT INTO users (full_name, email, phone, password_hash, role, email_verified)
       VALUES ($1,$2,$3,$4,'patient', FALSE) RETURNING id, full_name, email, role, must_change_password`,
    ...
    // Account exists but is unusable until they enter the code — no JWT
    // issued yet. issueVerificationCode both stores the code and emails it
    // (or logs it to the console if SMTP isn't configured, same fallback
...
// POST /api/auth/verify-email — completes self-signup by checking the
// emailed code. On success, issues the JWT (this is the actual "login"
// moment for a brand new account — nothing before this point grants access).
router.post('/verify-email', async (req, res) => {
  ...
  try {
    ...
    const result = await checkVerificationCode(client, user_id, code);
    if (!result.ok) {
      await client.query('ROLLBACK');
    ...
    const userResult = await client.query(
      `UPDATE users SET email_verified = TRUE WHERE id = $1
       RETURNING id, full_name, email, role, must_change_password`,
      [user_id]
    );
    const user = userResult.rows[0];
    if (!user) {
    ...
    const patientResult = await client.query('SELECT * FROM patients WHERE user_id = $1', [user_id]);
...
router.post('/resend-verification-code', async (req, res) => {
  ...
  try {
    ...
    if (user.email_verified) return res.status(400).json({ error: 'This account is already verified — just sign in.' });

    // Simple resend cooldown so this can't be used to spam someone's inbox.
    ...
    if (recent.rows.length > 0) {
      ...
      if (secondsSince < 60) {
        return res.status(429).json({ error: `Please wait ${Math.ceil(60 - secondsSince)}s before requesting another code.` });
      }
    }
